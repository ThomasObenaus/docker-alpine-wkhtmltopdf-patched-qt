...
build.golang-alpine: sep ## Builds the golang-alpine image with wkhtmltopdf (with patched qt)
	@echo "--> Build thobe/wkhtmltopdf:golang1.14-alpine Docker image"
	@docker build -f golang_alpine.Dockerfile -t thobe/wkhtmltopdf:golang1.14-alpine .

build.base: sep ## Build the base image that contains the compiled wkhtmltopdf with patched qt
...
push.golang-alpine: sep ## Pushes the golang-alpine image with wkhtmltopdf (with patched qt)
	@echo "--> Push thobe/wkhtmltopdf:golang1.14-alpine Docker image"
	@docker push thobe/wkhtmltopdf:golang1.14-alpine
